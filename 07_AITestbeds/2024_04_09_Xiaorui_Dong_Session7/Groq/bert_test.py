...
def evaluate_bert_tiny(rebuild_policy=None, should_execute=True):
    ...
    print(f"probabilities: {torch.nn.functional.softmax(outputs.float(), dim=-1)}")
    
    class_label = torch.argmax(outputs)
    print(f"class label: {class_label} ({'positive' if class_label else 'negative'})")
